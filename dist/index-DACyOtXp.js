import { P as nt, m as j, n as v, T as st, o as Ae, s as Be, p as it, t as c, q as E, r as ot, L as le, u as at, v as lt, w as ht, x as ft, e as _, l as P, y as T, z as ut, f as Ie, i as dt, A as pt, C as ct, B as mt, D as he, F as gt } from "./main-sutDqeT4.js";
import { html as kt, htmlCompletionSource as Lt } from "./index-O1vNNJCI.js";
class $ {
...
function fe(t, e, r) {
  ...
  if (r.indent >= r.baseIndent + 4)
    return !1;
  let n = (t.type == u.OrderedList ? ie : se)(r, e, !1);
  return n > 0 && (t.type != u.BulletList || ne(r, e, !1) < 0) && r.text.charCodeAt(r.pos + n - 1) == t.value;
}
const Ee = {
  [u.Blockquote](t, e, r) {
...
function ne(t, e, r) {
  ...
  return r && t.next == 45 && Ne(t) > -1 && t.depth == e.stack.length && e.parser.leafBlockParsers.indexOf(Te.SetextHeading) > -1 || n < 3 ? -1 : 1;
}
function Pe(t, e) {
...
function se(t, e, r) {
  return (t.next == 45 || t.next == 43 || t.next == 42) && (t.pos == t.text.length - 1 || x(t.text.charCodeAt(t.pos + 1))) && (!r || Pe(e, u.BulletList) || t.skipSpace(t.pos + 2) < t.text.length) ? 1 : -1;
}
function ie(t, e, r) {
  ...
  return n == t.pos || n > t.pos + 9 || s != 46 && s != 41 || n < t.text.length - 1 && !x(t.text.charCodeAt(n + 1)) || r && !Pe(e, u.OrderedList) && (t.skipSpace(n + 1) == t.text.length || n > t.pos + 1 || t.next != 49) ? -1 : n + 1 - t.pos;
}
function ve(t) {
  if (t.next != 35)
    return -1;
...
const F = {
  ...
  FencedCode(t, e) {
    ...
    if (r < 0)
      return !1;
    let n = t.lineStart + e.pos, s = e.next, i = r - e.pos, o = e.skipSpace(r), a = ue(e.text, e.text.length, o), l = [g(u.CodeMark, n, n + i)];
  ...
  ATXHeading(t, e) {
    ...
    if (r < 0)
      return !1;
    let n = e.pos, s = t.lineStart + n, i = ue(e.text, e.text.length, n), o = i;
  ...
  HTMLBlock(t, e) {
    ...
    o && t.nextLine();
    let a = s == ye ? u.CommentBlock : s == Oe ? u.ProcessingInstructionBlock : u.HTMLBlock, l = t.prevLineEnd();
...
`);
  ...
  */
  ...
  addGaps(e) {
    return this.ranges.length > 1 ? ze(this.ranges, 0, e.topNode, this.ranges[0].from, this.reusePlaceholders) : e;
  }
  /**
  @internal
  */
...
function ze(t, e, r, n, s) {
  ...
  for (let h = r.firstChild; h; h = h.nextSibling) {
    ...
    m ? d = m : h.to + n > i ? (d = ze(t, e, h, n, s), f(h.to + n, !1)) : d = h.toTree(), o.push(d), a.push(p - l);
  }
...
const $t = {
  parseInline: [{
    ...
    parse(t, e, r) {
      ...
      } else s[3] ? i = Se(t.text, n) : (i = Se(t.text, n + s[0].length), i > -1 && s[0] == "xmpp:" && (be.lastIndex = i, s = be.exec(t.text), s && (i = s.index + s[0].length)));
...
function Jt(t, e) {
  return (r) => {
    if (r && t) {
      let n = null;
      if (r = /\S*/.exec(r)[0], typeof t == "function" ? n = t(r) : n = he.matchLanguageName(t, r, !0), n instanceof he)
...
const Yt = ({ state: t, dispatch: e }) => {
  let r = _(t), { doc: n } = t, s = null, i = t.changeByRange((o) => {
    ...
    for (; k > l.from && /\s/.test(l.text.charAt(k - l.from - 1)); )
      k--;
    return L = ae(L, t), Wt(h.node, t.doc) && (L = xe(f, t, l) + t.lineBreak + L), d.push({ from: k, to: a, insert: t.lineBreak + L }), { range: P.cursor(k + L.length + 1), changes: d };
  });
